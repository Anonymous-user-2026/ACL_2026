...
def mf1(targets: list[np.ndarray] | np.ndarray,
    ...
    Args:
        ...
        predicts: Predictions array (model outputs)
...
def acc_func(trues, preds):
    # mean absolute error per trait → invert to accuracy
    acc = []
    for i in range(5):
...
def ccc(y_true, y_pred):
    """
    Calculates concordance correlation coefficient (CCC) between two series.
...
__all__ = [
    # emotions (multilabel CMU)
    "mf1", "uar",
    # personality (regression)
    "acc_func", "ccc",
    # AH / BAH (binary classification)
    "mf1_ah", "uar_ah",
]
